...
def get_data_loaders(dataset_path, batch_size=64):
    ...
    train_dataset = TIFDataset(image_dir, train_df, transform=train_transform)
    val_dataset = TIFDataset(image_dir, val_df, transform=val_transform)
    test_dataset = TIFDataset(image_dir, test_df, transform=val_transform)
...
def train(model, train_loader, val_loader, device, output_dir, epochs=10):
    # Extract all labels from the train_loader
    train_labels = []
    for _, labels in train_loader:
        train_labels.extend(labels.numpy())  # Convert to numpy array

    # Get all unique classes in the dataset
    unique_classes = np.unique(train_labels)

    # Ensure all classes are present in the training data
    if len(unique_classes) < 7:  # Replace 7 with the number of classes in your dataset
        raise ValueError(f"Not all classes are present in the training data. Found: {unique_classes}")

    # Compute class weights
    class_weights = compute_class_weight(
        class_weight='balanced',
        classes=unique_classes,  # Pass all unique classes
        y=train_labels
    )
...
#TODO:
#Data augmentation that adds new images to minority classes
...
def main():
    ...
    model.fc = nn.Sequential(
        ...
        nn.Linear(256, 7)  # Modify output layer for 7 classes
    )
